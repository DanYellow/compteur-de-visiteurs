...
router.get(['/lieu', '/lieu/:placeId'], requireRoleMiddleware(), async (req, res) => {
    let place = null
...
}).post(['/lieu', '/lieu/:placeId'], requireRoleMiddleware(), async (req, res, next) => {
...
}).post(['/lieu/suppression'], requireRoleMiddleware(), async (req, res) => {
    try {
